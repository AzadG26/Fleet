...
/* Add new scrap material (global rate) */
...
router.get("/global", async (req, res) => {
  try {
    const result = await pool.query(`SELECT * FROM scrap_types ORDER BY material_type ASC`);
...
/* Update global rate and adjust vendor rates preserving rate_offset */
router.post("/update-global", async (req, res) => {
  ...
  try {
    ...
    const vendorRates = await client.query(
      `SELECT id, vendor_id, rate_offset FROM vendor_rates WHERE scrap_type_id = $1`,
      [scrap_type_id]
    );
    ...
    for (const v of vendorRates.rows) {
      ...
      await client.query(
        `UPDATE vendor_rates SET vendor_rate = $1 WHERE id = $2`,
        [new_vendor_rate, v.id]
      );
      updatedVendors.push({ vendor_rate_id: v.id, vendor_id: v.vendor_id, new_vendor_rate, rate_offset: v.rate_offset });
    }
...
/* Add new vendor (feriwala / kabadiwala) */
router.post("/add-vendor", async (req, res) => {
  try {
    const { name, vendor_type } = req.body;
    if (!name || !vendor_type) return res.status(400).json({ error: "name and vendor_type required" });
...
/* Set vendor specific rate (inserts vendor_rates entry with rate_offset) */
router.post("/set-vendor-rate", async (req, res) => {
  try {
    ...
    if (!vendor_id || !scrap_type_id || vendor_rate == null) {
      return res.status(400).json({ error: "vendor_id, scrap_type_id, vendor_rate required" });
    }
    ...
    if (globalRateRes.rowCount === 0) return res.status(404).json({ error: "material not found" });

    const rate_offset = Number(vendor_rate) - Number(globalRateRes.rows[0].global_rate);
...
router.post("/update-vendor", async (req, res) => {
  try {
    ...
    if (!vendor_id || !scrap_type_id || new_vendor_rate == null) {
      return res.status(400).json({ error: "vendor_id, scrap_type_id, new_vendor_rate required" });
    }
    ...
    const rate_offset = Number(new_vendor_rate) - Number(globalRateRes.rows[0].global_rate);
